...
#define PRECISION 5
#define MAX_IMPROVEMENT_ATTEMPTS 1000000
...
#pragma omp parallel for shared(global, global_value)
    for (int it = 0; it < 50; it++) {
        ...
        while (T > min_temperature) {
            ...
            for (int inner_iterations = 0; inner_iterations < MAX_INNER_ITERATIONS;
                inner_iterations++) {
                ...
                else if (distribution(random_generator) <
                    std::exp(-std::abs(vn_value - vc_value) / T)) {
                    vc = vn;
                    vc_value = vn_value;
                    break;
                }
...
#pragma omp critical // This ensures that this section is executed by one thread
        // at a time
        {
...
int main() {
    ...
    for (int dim : dimensions) {
        ...
        // michalewicz, michalewicz_bounds, dim, BEST, mutation_rate); // here
        ...
        // Run Simulated Annealing (Hill Climbing - Best Improvement) for
        // Rastrigin's length = calculate_length(rastrigin_bounds, dim); auto
        // rastrigin_bits_sa = generate_random_bits(length); auto
        // rastrigin_result_bits_sa = hybrid_simulated_annealing(rastrigin_bits_sa,
        // rastrigin, rastrigin_bounds, dim); double rastrigin_result_sa =
        // calculate_function(rastrigin_result_bits_sa, rastrigin, rastrigin_bounds,
        // dim); printf("Rastrigin's (Simulated Annealing - Best Improvement):
        // %f\n", rastrigin_result_sa);
          // auto start = std::chrono::high_resolution_clock::now();
          // length = calculate_length(schwefel_bounds, dim);
          // auto schwefel_bits = generate_random_bits(length);
          // auto schwefel_result_bits = hybrid_simulated_annealing(
          //     schwefel_bits, schwefel, schwefel_bounds, dim);
          // double schwefel_result = calculate_function(
          //     schwefel_result_bits, schwefel, schwefel_bounds, dim);
          // printf("schwefel's (Simulated Annealing - Best Improvement): %f\n",
          //        schwefel_result);
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            auto start = std::chrono::high_resolution_clock::now();
            auto michalewicz_bits = generate_random_bits(length);
            auto michalewicz_result_bits =
                hill_climbing(michalewicz_bits, michalewicz, michalewicz_bounds, dim, WORST,
                    MUTATION_RATE); // here
            double michalewicz_result = calculate_function(
                michalewicz_bits, michalewicz, michalewicz_bounds, dim);
